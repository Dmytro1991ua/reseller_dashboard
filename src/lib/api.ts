...
export const createCryptoTopup = (body: { amount_cents: number; currency: string; network: string }) =>
...
export const getMetricsDestinations = (planId: string, hours = 24, limit = 30) =>
  apiRequest<DestinationsData>(`/plans/${planId}/metrics/destinations`, { searchParams: { hours, limit } })
...
export const getMetricsErrorMessages = (planId: string, hours = 24) =>
  apiRequest<ErrorMessagesData>(`/plans/${planId}/metrics/error-messages`, { searchParams: { hours } })
...
export const startInvestigation = (planId: string, complaint: string) =>
  apiRequest<{ investigation_id: string; status: string; estimated_seconds: number; poll_url: string }>(
    `/investigate/${planId}`,
    { method: 'POST', body: { complaint } }
  )
...
export const createSubUser = (body: { email: string; name: string; initial_balance_cents?: number }) =>
